...
public class Sorter {
     ...
     */
    private static ThreadPool pool = new ThreadPool(QUEUE_SIZE, N_THREADS);

//    private static CustomExecutor pool = new CustomExecutor(N_THREADS);
        ...
        private void quicksort(int pLeft, int pRight) throws InterruptedException {
            if (pLeft < pRight) {
                ...
                if (count.get() >= FALLBACK * N_THREADS) {
                    ...
                    count.getAndAdd(2);
                    pool.submitTask(new QuicksortRunnable<T>(values, pLeft, storeIndex - 1, count));
                    pool.submitTask(new QuicksortRunnable<T>(values, storeIndex + 1, pRight, count));
                }
            }
        }
